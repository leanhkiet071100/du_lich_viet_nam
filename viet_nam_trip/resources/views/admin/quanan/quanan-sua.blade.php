        ...
        <div class="row">
            <div class="col-md-12">
                <div class="main-card mb-3 card">
                    <div class="card-body">
                        <form method="post" enctype="multipart/form-data" action="{{ route('admin.quan-an.update', ['id'=>$quan_an->id]) }}">
                            @csrf
                                        ...
                                        <li class="float-left d-inline-block mr-2 mb-2" style="width: 32%;">
                                            ...
                                            <div style="width: 100%; max-height: 100%; overflow: hidden;"
                                                ...
                                                <img style="width: 100%; cursor: pointer;" class="thumbnail"
                                                    ...
                                                    src="{{ URL($quan_an->hinh_quan_an ?? 'assets/img/add-image-icon.jpg') }}" alt="Add Image">
                            ...
                            <div class="position-relative row form-group">
                                ...
                                <div class="col-md-9 col-xl-8">
                                    <input name="ten" id="ten" placeholder="Tên quán ăn" type="text" class="form-control" value="{{ old('ten') ?? $quan_an->ten_quan_an }}">
                                    <div class="text-center">
                                        @error('ten')
                                            <span style="color:red"> {{ $message }}</span>
                                        @enderror
                            ...
                            <div class="position-relative row form-group">
                                ...
                                <div class="col-md-9 col-xl-8">
                                    <input name="so-dien-thoai" id="so-dien-thoai" placeholder="Số điện thoại" type="text" class="form-control" value="{{ old('so-dien-thoai') ?? $quan_an->so_dien_thoai }}" onkeypress="return isNumberKey(event)">
                                    <div class="text-center">
                                        @error('so-dien-thoai')
                                            <span style="color:red"> {{ $message }}</span>
                                        @enderror
                                    ...
                                    <div class="">
                                        <select name="dia_diem_id" class="form-select select-dia-diem-id" aria-label="Default select example" data-type="dia_diem_id">
                                            ...
                                            @foreach ($ls_dia_diem as $key => $value)
                                                <option {{ (old('dia_diem_id') ?? $quan_an->dia_diem_id) == $value->id ? 'selected' : '' }} value="{{ $value->id }}">{{ $value->ten_dia_diem }}</option>
                                            @endforeach
                         ...
                         <div class="position-relative row form-group">
                                            ...
                                            <select name="tinh" class="form-select js-location" aria-label="Default select example" data-type="tinh" id="js-location-tinh js-location">
                                                ...
                                                @if ( old('huyen') == null)
                                                    @foreach ($tinh as $key => $value)
                                                        <option {{ $quan_an->tinh == $value->ten ? 'selected' : '' }} value="{{ $value->id }}">{{ $value->ten }}</option>
                                                    @endforeach
                                                @else
                                                    @foreach ($tinh as $key => $value)
                                            ...
                                            <select name="huyen" id="js-location-huyen" class="form-select js-location" aria-label="Default select example" data-type="huyen">
                                                @if ( old('huyen') == null)
                                                    @foreach ($ls_huyen_tinh as $key => $value)
                                                        <option {{ $quan_an->huyen == $value->ten ? 'selected' : '' }} value="{{ $value->id }}">{{ $value->ten }}</option>
                                                    @endforeach
                                                @else
                                            ...
                                            <select name="xa"  class="form-select js-location-xa" aria-label="Default select example" data-type="xa" id="js-location-xa">
                                                @if ( old('xa') == null)
                                                    @foreach ($ls_xa_huyen as $key => $value)
                                                        <option {{ $quan_an->xa == $value->ten ? 'selected' : '' }} value="{{ $value->id }}">{{ $value->ten }}</option>
                                                    @endforeach
                                                @else
                            ...
                            <div class="position-relative row form-group mb-1">
                                <div class="col-md-9 col-xl-8 offset-md-2">
                                    <a href="{{ route('admin.quan-an.index') }}" class="border-0 btn btn-outline-danger mr-1">
