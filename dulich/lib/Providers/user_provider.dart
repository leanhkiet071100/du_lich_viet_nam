...
class UserProvider {
  ...
  static Future<bool> isLogged() async {
    ...
    final response = await http
        .get(Uri.parse("http://192.168.100.49:8000/api/login"), headers: {
      'Content-type': 'application/json',
      'Accept': 'application/json',
      'Authorization': 'Bearer $token',
    });
  ...
  static Future<bool> login(String email, String password) async {
    final response =
        await http.post(Uri.parse("http://192.168.100.49:8000/api/login"),
            body: ({
              'email': email,
              'password': password,
            }));
  ...
  static Future<bool> updateInfor(
    ...
    var token = await getToken();
    final response = await http.post(Uri.parse('user/update-infor'),
        body: jsonEncode({
          'hoTen': hoTen,
          'email': email,
          'soDienThoai': soDienThoai,
  ...
  static Future<bool> changePassword(
    ...
    var token = await getToken();
    final response = await http.post(Uri.parse('user/change-password'),
        body: jsonEncode({
          'password': oldPass,
  ...
  static Future<bool> logout() async {
    var token = await getToken();
    final response = await http
        .post(Uri.parse('http://192.168.100.49:8000/api/logout'), headers: {
      'Content-type': 'application/json',
      'Accept': 'application/json',
      'Authorization': 'Bearer $token',
    });
